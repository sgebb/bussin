...
window.msalHelper = {
    ...
    loginRedirect: async function (scopes, tenantId) {
        try {
            const msalInstance = await initPopupMsal();
            const request = {
                scopes: Array.isArray(scopes) ? (scopes.length > 0 ? scopes : ["https://management.azure.com/user_impersonation"]) : [scopes || "https://management.azure.com/user_impersonation"],
                authority: tenantId ? `https://login.microsoftonline.com/${tenantId}` : undefined,
                prompt: 'select_account'
            };
            console.log('[msal-helper] loginRedirect triggered with prompt=select_account');
            await msalInstance.loginRedirect(request);
        } catch (error) {
            console.error('[msal-helper] Redirect error:', error);
        }
    }
};
